...
{
	BindlessResourcesRenderPass::BindlessResourcesRenderPass(const RenderDevice_ptr& device, const RenderResources_ptr& resources, const RenderSwapchain_ptr& swapchain) :
		RenderPassBase(device, resources, swapchain->GetWidthF(), swapchain->GetHeightF()) {
	}

	BindlessResourcesRenderPass::~BindlessResourcesRenderPass()
	{
	...
	void BindlessResourcesRenderPass::Initialize()
	{
		// attachments
		...
		// render subpass
		VkAttachmentReference renderSubpassColorAttachmentReference = Utility::Vulkan::CreateInfo::attachmentReference(static_cast<uint32_t>(Attachments::MainRenderTarget), VkImageLayout::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
		subpassDescriptions.push_back(Utility::Vulkan::CreateInfo::subpassDescription_color(renderSubpassColorAttachmentReference));
	...
	void BindlessResourcesRenderPass::BeginRender(const VkCommandBuffer& commandBuffer, const uint32_t& currentBufferIndex)
	{
	...
	void BindlessResourcesRenderPass::EndRender(const VkCommandBuffer& commandBuffer)
	{
		vkCmdEndRenderPass(commandBuffer);
	}

	void BindlessResourcesRenderPass::createFrameBuffers()
	{
