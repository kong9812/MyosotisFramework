...
{
	TerrainRenderPass::TerrainRenderPass(const RenderDevice_ptr& device, const RenderResources_ptr& resources, const RenderSwapchain_ptr& swapchain) :
	...
	void TerrainRenderPass::Initialize()
	{
		...
		std::vector<VkSubpassDependency> dependencies = {
			...
			Utility::Vulkan::CreateInfo::subpassDependency(
				VK_SUBPASS_EXTERNAL,
				static_cast<uint32_t>(SubPass::Terrain),
				...
				// 外部 -> Terrain（ColorAttachment 初期化）
				Utility::Vulkan::CreateInfo::subpassDependency(
					VK_SUBPASS_EXTERNAL,
					static_cast<uint32_t>(SubPass::Terrain),
	...
	void TerrainRenderPass::BeginRender(const VkCommandBuffer& commandBuffer, const uint32_t currentBufferIndex)
	{
	...
	void TerrainRenderPass::EndRender(const VkCommandBuffer& commandBuffer)
	{
		vkCmdEndRenderPass(commandBuffer);
	}

	void TerrainRenderPass::createFrameBuffers()
	{
