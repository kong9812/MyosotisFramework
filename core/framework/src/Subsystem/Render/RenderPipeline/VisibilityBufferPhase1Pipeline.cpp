...
{
	VisibilityBufferPhase1Pipeline::~VisibilityBufferPhase1Pipeline()
	{
	...
	void VisibilityBufferPhase1Pipeline::Initialize(const RenderResources_ptr& resources, const VkRenderPass& renderPass)
	{
		...
		for (uint32_t i = 0; i < AppInfo::g_maxInFlightFrameCount; i++)
		{
			...
			pushConstant[i].phase = 1;
		}
	}

	void VisibilityBufferPhase1Pipeline::BindCommandBuffer(const VkCommandBuffer& commandBuffer, const uint32_t frameIndex, const uint32_t vbDispatchInfoCount)
	{
		{// Phase1Render
			...
			pushConstant[frameIndex].pcVBDispatchInfoCount = vbDispatchInfoCount;
	...
	void VisibilityBufferPhase1Pipeline::Resize(const RenderResources_ptr& resources)
	{
		for (uint32_t i = 0; i < AppInfo::g_maxInFlightFrameCount; i++)
		{
			const Image& hiZDepth = resources->GetHiZDepthMap(i);
	...
	void VisibilityBufferPhase1Pipeline::prepareRenderPipeline(const RenderResources_ptr& resources, const VkRenderPass& renderPass)
	{
		// push constant
