...
{
	InteriorObject::InteriorObject() : StaticMesh()
	{
		m_name = "プリミティブジオメトリ";
	}
	...
	void InteriorObject::BindCommandBuffer(const VkCommandBuffer& commandBuffer, const RenderPipelineType& pipelineType)
	{
		__super::BindCommandBuffer(commandBuffer, pipelineType);
	}

	glm::vec4 InteriorObject::GetCullerData()
	{
