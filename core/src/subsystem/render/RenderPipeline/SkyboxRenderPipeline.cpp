...
{
	SkyboxRenderPipeline::SkyboxRenderPipeline(RenderDevice_ptr device, RenderResources_ptr resources, VkRenderPass renderPass)
	{
		m_device = device;
	...
	{
		{// pipeline
			shaderObject.skyboxRenderShaderBase.pipelineLayout = m_pipelineLayout;
			shaderObject.skyboxRenderShaderBase.pipeline = m_pipeline;
		}
		...
		VK_VALIDATION(vkAllocateDescriptorSets(*m_device, &descriptorSetAllocateInfo, &shaderObject.skyboxRenderShaderBase.descriptorSet));

		std::vector<VkWriteDescriptorSet> writeDescriptorSet = {};
		VkDescriptorImageInfo descriptorImageInfo{};
		descriptorImageInfo = Utility::Vulkan::CreateInfo::descriptorImageInfo(shaderObject.standardUBO.cubemap.sampler, shaderObject.standardUBO.cubemap.image.view, VkImageLayout::VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

		// write descriptor set
		writeDescriptorSet = {
			Utility::Vulkan::CreateInfo::writeDescriptorSet(shaderObject.skyboxRenderShaderBase.descriptorSet, 0, VkDescriptorType::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, &shaderObject.standardUBO.buffer.descriptor),
			Utility::Vulkan::CreateInfo::writeDescriptorSet(shaderObject.skyboxRenderShaderBase.descriptorSet, 1, VkDescriptorType::VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &descriptorImageInfo),
		};
		vkUpdateDescriptorSets(*m_device, static_cast<uint32_t>(writeDescriptorSet.size()), writeDescriptorSet.data(), 0, nullptr);
	}

	void SkyboxRenderPipeline::prepareDescriptors()
	{
	...
	void SkyboxRenderPipeline::prepareRenderPipeline(RenderResources_ptr resources, VkRenderPass renderPass)
	{
		// [pipeline]layout
